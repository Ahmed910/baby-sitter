	...
	<div id="step_locales" class="content">
        ...
        <div class="row">
            <div class="form-group col-md-12">
                ...
                <select name="day" class = 'select2 w-100'>
                <option value="">{{trans('dashboard.available_day.available_day')}}</option>
                    @foreach ($day_keys as $day)
                    <option value="{{ $day }}" {{ (isset($available_day) && $available_day->day == $day) ? 'selected':'' }}>{{ trans('dashboard.day_keys.'.$day) }}</option>
                    @endforeach
                </select>
        ...
        <div class="row">
	        <div class="form-group col-md-12">
				...
				{!! Form::select('district_id', $districts, null , ['class' => 'select2 w-100' , 'placeholder' => trans('dashboard.district.district') , 'id' => 'modern-district']) !!}
	        </div>
	    </div>
