...
class CompleteOrderNotification extends Notification implements ShouldBroadcast
{
    use Queueable;
    public $booking;
    public $via;
    ...
    //     ])->content([
    //         'title'=>trans('dashboard.notification.order_has_been_completed_title',[],$notifiable->current_lang),
    //         'body'=> trans('dashboard.notification.order_has_been_completed_body',['body' => auth()->user()->name ?? auth()->user()->phone],$notifiable->current_lang),
    //         'sender_data' => new SenderResource(auth('api')->user()),
    //         'sound'        => '', // Optional
    //         'icon'         => '', // Optional
    ...
    public function toBroadcast($notifiable)
    {
        ...
        return new BroadcastMessage([
            'title'=>trans('dashboard.notification.order_has_been_completed_title',[],$notifiable->current_lang),
            'body'=> trans('dashboard.notification.order_has_been_completed_body',[],$notifiable->current_lang) . auth('api')->user()->name??auth('api')->user()->phone,
            'notify_type'=>'complete_order',
            'route' => route('dashboard.orders.show',$this->booking->id),
            'order_id' => optional($this->booking)->id,
        ]);
    }
