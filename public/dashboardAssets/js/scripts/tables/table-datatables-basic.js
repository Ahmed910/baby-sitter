...
$(function () {
  ...
  var dt_basic_table = $('.datatables-basic'),
      dt_custom_table = $('.datatables-custom'),
  ...
  if (dt_custom_table.length) {
    var dt_basic = dt_custom_table.DataTable({
      // ajax: assetPath + 'data/table-datatable.json',
      // columns: [
      //   { data: 'responsive_id' },
      //   { data: 'id' },
      ...
      buttons: [
        ...
        {
          text: feather.icons['plus'].toSvg({ class: 'mr-1 font-small-2'}) + dt_custom_table.data('create_title'),
          className: 'create-new btn btn-primary',
          attr: {
            'onclick': `location.href="${dt_custom_table.data('create_link')}"`,
          },
          init: function (api, node, config) {
            $(node).removeClass('btn-secondary');
          }
    ...
    $('div.head-label').html(`<h4 class="mb-0">${dt_custom_table.data('title')}</h4>`);
  }
